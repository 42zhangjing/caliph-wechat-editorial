...
- `roast`：毒舌版。以已经审计的事实骨架、引用和统计为底稿，重写成更锋利、更好笑、更有封面感的评论；不把普通版换几个形容词就交差。
...
毒舌版的写作顺序是：先基于当前素材写一版足够尖锐的内部草稿，再执行红线审校；不要一开始就把每句话写成温吞的免责声明。最终成稿仍要满足以下动作：

1. 报头标题自动带 `毒舌版` 后缀；开场用戏谑口吻点出当天最荒诞的转折或反差。
2. 每个故事先说清事实，再把槽点落在公开观点、反复横跳、离谱推理或现场戏剧性上；标题可以更损，正文不能退化成段子墙。
3. 逐字引用可用 `quotes[].comment` 配一条短编辑点评；点评不是群友原话，必须与引用分开，不能篡改或借题发挥。
4. 人物卡在 roast 模式显示为「本期角色 · 不留情面版」；优先使用 `people[].roast_note` 写本期的反差或槽点。没有足够槽点的人只给轻调侃，不按发言数硬凑攻击。
5. 默认页脚为「本简报由一个没有感情的 AI 自动生成，如有冒犯，概不负责。」；用户明确指定其它页脚时才覆盖。

详细写法、反例与红线见 [references/editorial-workflow.md](references/editorial-workflow.md) 的“毒舌版”。
...
毒舌版在普通版事实骨架完成后再重写；`quotes[].text` 始终保留原文，尖锐点评写入 `quotes[].comment`。人物卡的普通 `note` 与 roast 专用 `roast_note` 可以并存，renderer 仅在 roast 输出中优先显示后者。
...
毒舌的关键不是多加形容词，而是把“公开说了什么、随后做了什么、结果多荒诞”放进同一个镜头。写作顺序固定为：先基于普通版素材写一份够锋利的内部草稿，再单独做红线审校；不要在下笔时就把每个槽点磨成温吞水。

成稿必须落实到版面字段：

- 报头标题自动带 `毒舌版`，Lead 直接点出本期最戏剧性的转折；
- 每个有明确槽点的故事，至少在标题、正文或 `quotes[].comment` 中落下一处具体点评；原话仍逐字保留；
- 入选人物若有本期可见的反差、反复横跳或现场行为，填写 `roast_note`；没有素材时温和一句或不入选，绝不硬造；
- renderer 会把人物栏标为「不留情面版」，并在未自定义 footer 时使用 AI 免责声明；
- 不碰外貌、身体、健康、家庭、私人关系；不从时间戳推断作息或时区；不做诊断，不猜身份；只笑观点、公开行为和讨论本身。
...
默认 `page_mode: auto`。模型负责故事顺序、`priority` 和内容重要性；renderer 先用双栏故事卡的估算做兜底，再用真实 Chrome 测候选页的实际高度重排，避免把能并排容纳的两条故事拆成两张空页。故事区会占用页脚上方的可用高度；多行卡片纵向拉开，单张落在最后一行时跨栏收束，留白成为版式呼吸而不是页脚后的一大片断层。
...
渲染后必须做浏览器 preflight：检查每个 A3 `.page` 的真实 `scrollHeight` 与 `clientHeight`。测量以 `DOMContentLoaded`、字体与两帧 layout 稳定为准，preflight 专用副本会用占位图替换远程资源，不等待 `window.load` 或远程 CALIPH Logo；正式 HTML / PNG / PDF 仍保留 Logo。自动模式会在人物、统计和 closing 加入最后一页后再次测量，必要时把最后一条故事移到该页；出现 overflow 时停止正式出版，调整分页或内容后再导出；不允许依赖 `overflow:hidden` 静默裁字。
